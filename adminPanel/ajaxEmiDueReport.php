...
$toDates= $todate[2].'-'.$todate[1].'-'.$todate[0];
		...
		if($_SESSION['userType']=="ADMIN")
		{
		$query=" SELECT * FROM loans inner join loanemi on loans.loanId=loanemi.loanId and loans.deleted='0' and loanemi.emiNo =(SELECT max(emiNO) from loanemi where loans.loanId=loanemi.loanId) and loanemi.ndd between '$fromDates'  and '$toDates' and loanemi.emiStatus !='PRE' and loanemi.branchCode='$branchTableId'";
		}
		else
		{
				$query=" SELECT * FROM loans inner join loanemi on loans.loanId=loanemi.loanId and  loans.deleted='0' and loanemi.emiNo =(SELECT max(emiNO) from loanemi where loans.loanId=loanemi.loanId) and loanemi.ndd between '$fromDates'  and '$toDates' and loanemi.emiStatus !='PRE'  and loans.branchCode='$branchId'";
		}	
